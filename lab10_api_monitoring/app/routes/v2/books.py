...
book_logger = logging.getLogger(__name__)
...
def get_book(id):
    ...
    """
    book_logger.info(f"Yêu cầu truy xuất sách với ID: {id} **(Trong module v2/books.py)**")
    try:
        book = books_collection.find_one({"_id": ObjectId(id)})
    except:
        book_logger.error(f"LỖI: ID sách không hợp lệ: {id}")
        return jsonify({"error": "Invalid book ID"}), 400

    if not book:
        book_logger.error(f"LỖI: Không tìm thấy sách có ID={id}")
